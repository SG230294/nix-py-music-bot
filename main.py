...
class MyClient(discord.Client):
  ...
  def play_from_queue(self, message):
    ...
    if len(globalQueue.get(message.guild.id)) > 0:
      ...
      if channel:
        ...
        try:
          source = discord.FFmpegOpusAudio("track.webm")
  ...
  async def show_queue(message):
    channel = client.get_channel(message.channel.id)
    if globalQueue.get(message.guild.id) is not None:
      text_queue = '## Очередь:' + '\n'
      i = 0
      for track in globalQueue.get(message.guild.id):
        i += 1
        text_queue += '' + str(i) + '. [' + track.get(
            'title') + '](<' + track.get('link') + '>)' + '\n'
    else:
  ...
  async def on_message(self, message):
    ...
    if message.content.startswith(f'{prefix}dice'):
      ...
      try:
        ...
        await channel.send(author + " бросает кости и выкидывает:   " + die1 +
                           "   " + die2)
      except:
